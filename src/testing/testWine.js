...
import ImgAraucanaAzul from "../../images/araucana-azul.png";
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://back-ribera-gl7lw5cfra-uc.a.run.app/api/wines?populate=mainBottleImage%2C%20secondaryImage%2C%20relatedImage&filters%5Bslug%5D${slug}`
        );
        setWineData(response.data.data[0].attributes);
                        ...
                        <div className="space-y-5 md:space-y-14">
                          ...
                          >
                            <span className="text-base md:text-[1.35rem] md:leading-[1.75rem]">
                              {wineData.familyName} <br />{" "}
                              <span className="font-bold">{wineData.name}</span>
                            </span>
                          </h1>
                          ...
                          >
                            {wineData.description[0].children[0].text}
                          </p>
                        </div>
                        {/* titular wine end */}
                      ...
                      <div className="border border-[#C4B27D] flex flex-col justify-between px-3 pt-3 md:border-0 md:p-0">
                        <div className="flex flex-col justify-center items-center gap-y-2 md:gap-y-4">
                          <Link
                            ...
                            className="text-white text-sm uppercase lg:text-lg border-b border-transparent hover:border-[#C4B27D] hover:text-[#C4B27D] transition duration-300 ease-in-out"
                          >
                            {t("wines.btnFooter")}
                          </Link>
